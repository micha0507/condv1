...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!preg_match("/^[a-zA-Z\s]+$/", trim($nombre))) {
            ...
            if ($allow_insert) {
                if ($action === 'add') {
                    ...
                    if ($stmt->execute()) {
                        $message = "<div class='alert alert-success' style='background:linear-gradient(90deg,#e6ffed,#d4f7e2);border:1px solid #2ecc71;color:#155724;padding:12px 16px;border-radius:6px;box-shadow:0 2px 6px rgba(46,204,113,0.15);font-weight:600;display:inline-block;'>Nuevo miembro añadido exitosamente.</div>";
                        // limpiar
                        $rif = $nombre = $apellido = $usuario = $pass = $email_propietario = "";
                        $tipo_rif = "V";
                    } else {
                        $message = "<div class='alert alert-danger'>Error: " . htmlspecialchars($conexion->error) . "</div>";
                    }
                    $stmt->close();
                } else { // update
                    ...
                    $stmt_up->bind_param("sssssss", $rif, $nombre, $apellido, $usuario, $pass, $email_propietario, $original_rif);
                    if ($stmt_up->execute()) {
                        $message = "<div class='alert alert-success' style='background:linear-gradient(90deg,#e6ffed,#d4f7e2);border:1px solid #2ecc71;color:#155724;padding:12px 16px;border-radius:6px;box-shadow:0 2px 6px rgba(46,204,113,0.15);font-weight:600;display:inline-block;'>Propietario actualizado correctamente.</div>";
                        // limpiar y salir de modo edición
                        $rif = $nombre = $apellido = $usuario = $pass = $email_propietario = "";
                        $tipo_rif = "V";
                        $is_edit = false;
                    } else {
                        $message = "<div class='alert alert-danger'>No se pudo actualizar: " . htmlspecialchars($conexion->error) . "</div>";
                    }
                    $stmt_up->close();
                }
            }
    ...
    $sql_get = "SELECT rif, nombre, apellido, usuario, pass, email_propietario FROM propietario WHERE rif = ?";
...
<body>
    ...
    <section class="principal">
        ...
        $sql_list = "SELECT rif, nombre, apellido, usuario, email_propietario FROM propietario ORDER BY nombre, apellido LIMIT ?, ?";
        $stmt_list = $conexion->prepare($sql_list);
        if ($stmt_list) {
            // bind_param requiere enteros como i
